<template>
  <section-component :title="'methods'" imageName="glitter">
    <div class="projects__container">
      <div class="project__card" @click="toggleProjectCard($event, true)" @focus="toggleProjectCard($event, true)" @blur="toggleProjectCard($event, false)">
        <div class="project__card-content__wrapper spacing-gap">
          ...
          <p class="project__card-subtitle text-xxs">Websites, web apps, native apps (Android)</p>
        </div>
      </div>
      <div class="project__card" @click="toggleProjectCard($event, true)" @focus="toggleProjectCard($event, true)" @blur="toggleProjectCard($event, false)">
        <div class="project__card-content__wrapper spacing-gap">
          <h2 class="project__card-title title">back-end</h2>
          <p class="project__card-subtitle text-xxs">(REST) APIs, database queries (CRUD), creating endpoints, data validation, authentication, authorization, migrations, seeders</p>
        </div>
      </div>
      <div class="project__card" @click="toggleProjectCard($event, true)" @focus="toggleProjectCard($event, true)" @blur="toggleProjectCard($event, false)">
        <div class="project__card-content__wrapper spacing-gap">
          <h2 class="project__card-title title">UX</h2>
          <p class="project__card-subtitle text-xxs">Accessibility, interviews, journey maps, personas, user flows, usability testing, info architecture, paper prototypes, empathy maps, user stories, wireframes</p>
        </div>
      </div>
      <div class="project__card" @click="toggleProjectCard($event, true)" @focus="toggleProjectCard($event, true)" @blur="toggleProjectCard($event, false)">
        <div class="project__card-content__wrapper spacing-gap">
          <h2 class="project__card-title title">hardware</h2>
          <p class="project__card-subtitle text-xxs">Rapid prototyping, soldering, robots, Arduino, Raspberry Pi, IoT, smart objects, 3D printing, laser cutting</p>
        </div>
      </div>
    </div>
  </section-component>
